...
from bork.utils import EventType
...
class UTG(object):
    ...
    def __init__(self, device, app, random_input, wear_device, device_paired=None):
        ...
        print('\eba\ utg. wear_device {}'.format(wear_device))
        print('\eba\ utg. wear_device %d' % wear_device)

        if ( wear_device == 1):
            print ("here!!")


    def add_transition(self, event, old_state, new_state, wear_events):
        ...
        # eba
        # print("\eba\ utg.py add_transition() - Wear Events ~> {}".format(wear_events))
        # print("\eba\ utg.py add_transition() - event ~> {}".format(event))
        # print("\eba\ utg.py add_transition() - tag ~> {}".format(event.tag))

        event_str = event.get_event_str(old_state)
        self.input_event_count += 1

        # no transition (old_state == new_state)
        ...
        # new transition (old_state != new_state)
        if (old_state.state_str, new_state.state_str) not in self.G.edges():
            self.G.add_edge(old_state.state_str, new_state.state_str, events={}, wear_events=wear_events, tag=event.tag)
    ...
    def __output_utg(self):
        ...
        # experimental
        # save a copy of the last utg.js, before overwrite it
        # FIXME: eba 6/19/19 Fails because the file does not exists yet.
        # shutil.copy(utg_file_path, os.path.join(self.device.output_dir, "utg-last.js"))
        utg_file = open(utg_file_path, "w")
        utg_nodes = []
        utg_edges = []

        # retrieve sensor list from the wearable device
        sensor_list = []
        ...
        for state_transition in self.G.edges():
            ...
            tag = self.G[from_state][to_state]["tag"]
            events = self.G[from_state][to_state]["events"]
            event_short_descs = []
            event_list = []
            # wearable
            wear_events = self.G[from_state][to_state]["wear_events"]
            wear_event_list = []
            ...
            # wear-events: populate list
            for wear_event in wear_events:

                # print("wear_event ** ~>")
                # print(wear_event)
                # print(wear_event["type"].to_name())

                # transition-id is redundantly stored here to facilitate the parsing to build the model for the wearable
                # FIXME: /eba/ 7/5/19 Remove call of active sensor, instead retrieve the status -- Done
                # FIXME: /eba/ 7/31/19 We don't need to make any distinction between the type.
                if wear_event["type"] == EventType.Sensor:
                    wear_event_list.append({
                        "event_id": "none",  # TODO: Determine the id
                        "event_str": "%s(Sensor=%s, Status=%s)" %
                                     (wear_event["type"].to_name(),
                                      wear_event["sensor_handle"],
                                      wear_event["sensor_status"]),
                        "event_type": wear_event["type"].to_name(),
                        "timestamp": wear_event["timestamp"],
                    ...
                    wear_event_list.append({
                        "event_id": "none",  # TODO: Determine the id
                        "event_str": "%s(component=%s, action=%s)" %
                                     (wear_event["type"].to_name(),
                                      wear_event["package"] + "/" + wear_event["component"],
                                      wear_event["action"]),
                        "event_type": wear_event["type"].to_name(),
        ...
        utg = {
            ...
            "first_activity": self.first_state_str,
    ...
    def get_sensors(self):
        if ( self.wear_device == 1 ):
            return self.device.get_sensor_list()
        elif ( self.wear_device == 2):
            return self.device_paired.get_sensor_list()

    def get_sensor_status(self):
        if (self.wear_device == 1):
            return self.device.get_sensor_status()
        elif (self.wear_device == 2):
            return self.device_paired.get_sensor_status()
