...
class DeviceState(object):
    ...
    def __init__(self, device, views, foreground_activity, activity_stack, background_services,
        ...
        # print("\eba\ device_state - serial: {} is_wearable: {} sensors: {}".format(device.serial, device.is_wearable, device.sensors))
    ...
    def __get_state_str_raw(self):
        ...
        # FIXME. Find out why the footprint of the state is different, even though that the states look the same
        # print("\eba\ __get_state_str_raw -> {}".format(self.device.humanoid))

        # NOTE: /dcsl/ The state footprint
        #       If humanoid is active:
        #
        #       else:
        #            The md5 hash of the list of all the views from the current state.


        if self.device.humanoid is not None:

            print("\eba\ __get_state_str_raw -> if")
            ...
            print("\eba\ __get_state_str_raw -> device.humanoid is None")
            ...
            footprint = "%s{%s}" % (self.foreground_activity, ",".join(sorted(view_signatures)))
            # print("\eba\ device_state.py get_state_str_raw -> signature \n\n{}\n".format(footprint))
            # print("\eba\ device_state.py get_state_str_raw -> state_st {}\n".format(md5(footprint)))

            # if self.device.is_wearable:
            #     print ("\eba\ device_state - sensor_status: {}".format(self.device.get_sensor_status()))
    ...
    def __get_view_signature(view_dict):
        ...
        # FIXME: eba 6/21/19 droidbot generates lot of useless events
        # Trying to solve this by removing the view_text from the signature
        view_text = "None"
    ...
    def get_possible_input(self):
        ...
        # FIXME: /eba/ 6/12/19 Dimensions and bounds of widget are unreal. In some cases the values are beyond the size
        # of the device's screen. Most probably because the main layout is scrollable.
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'clickable'):
                ...
                # print("\eba\ __get_possible_input -> TouchEvent: {}".format(self.views[view_id]))
    ...
    def __get_sensor_signature(self):
        """
        Get the signature for sensor activity in the device.
        This method should only be invoked if the device is a wearable.
        :return:
        """
        if self.device.is_wearable:
            pass