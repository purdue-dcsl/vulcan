...
class InputPolicy(object):
    ...
    def __init__(self, device, app):
        ...
        # wearable logcat
        self.wear_logger = None
        # paired device
        self.device_paired = None
        # wearable device
        self.wear_device = None
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def __init__(self, device, app, random_input, device_paired=None, wear_device=None):
        ...
        # wearables
        self.device_paired = device_paired
        self.wear_device = wear_device

        print ('\eba\ input_policy.py ---->')
        print ('\eba\ input_policy.py wear_device = {}'.format(wear_device))

        self.utg = UTG(device=device, app=app, random_input=random_input, wear_device=self.wear_device)
    ...
    def generate_event(self):
        ...
        # NOTE: /dcsl/ Here is where the magic lies.
        #       Once an event is generated, a transition is done in the UTG, which is the UI transition graph.
        ...
        # generates events based on the sub policy (e.g., naive, greedy, manual)
        if event is None:
    ...
    def __update_utg(self):
        if ( self.last_event != None):
            print("[eba] add_transition: %s to %s " % (self.last_event.get_event_str(self.last_state), self.last_event.get_event_str(self.current_state)))
        else:
            print("[eba] add_transition: %s to %s " % (str(self.last_state), str(self.current_state)))

        # FIXME. eba 6/11/19 Mappings should come from either device (main or paired)
        if self.wear_logger is None:
            mappings = []
            print("\eba\ input_policy.py __update_utg() - No mappings")
        else:
            mappings = self.wear_logger.get_mappings()
            print("\eba\ input_policy.py __update_utg() - mappings {}".format(mappings))

        self.utg.add_transition(self.last_event, self.last_state, self.current_state, mappings)
...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def __init__(self, device, app, random_input, search_method,
                 device_paired, wear_device):
        super(UtgGreedySearchPolicy, self).__init__(device, app, random_input, device_paired, wear_device)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.search_method = search_method
        ...
        # print ('\eba\ input_policy - utggreddy wear_device %d' % wear_device)

        # NOTE: /dcsl/ Wearable Device Logger.
        #       We are collecting informacion about the state in the wearable using logcat.
        #       wear_device indicates if the wearable is either self.device (1) or self.device_paired (2).
        #
        #       Disclaimer.
        #       The implementation is done just in the `Greedy Search Policy` to avoid any possible complication with
        #       heritage.

        self.device_paired = device_paired
        self.wear_device = wear_device

        self.logger.info("\eba\ before if")
        if wear_device == 1:
            self.logger.info("wear_device == 1")
            self.wear_logger = device.logcat
        elif wear_device == 2:
            self.logger.info("wear_device == 2")
            self.wear_logger = device_paired.logcat
        else:
            self.logger.info("wear_device == ?")
            self.wear_logger = None

        self.logger.info("\eba\ input_policy.py UtgGreedySearchPolicy :: __init__()")
        # self.logger.info("\eba\ wear_device {} ".format(wear_device))
        # self.logger.info("\eba\ wear_logger {} ".format(self.wear_logger))
        # self.logger.info("\eba\ device        {} ".format(device.logcat))
        # self.logger.info("\eba\ device_paired {} ".format(self.device_paired.logcat))

        # FIXME. eba 6/11/19 Retrieve the sensors from either device (main or paired)
        # self.logger.info("Sensor list from input_policy: %s" % self.wear_device.sensors)
    ...
    def generate_event_based_on_utg(self):
        ...
        print("\eba\ __generate_event_based_on_utg -> self.__event_trace: {}".format(self.__event_trace))
        ...
        # print("\eba\ __generate_event_based_on_utg -> possible_events: {}".format(possible_events))
        ...
        # FIXME eba 6/10/19 Apparently this is not working, since the app is not being restarted.
        stop_app_intent = self.app.get_stop_intent()
    ...
    def generate_event(self):
        ...
        while self.event_idx < len(self.event_paths) and \
            ...
            while curr_event_idx < len(self.event_paths):
                ...
                print('\eba\ replaying: {}'.format(event_path))
