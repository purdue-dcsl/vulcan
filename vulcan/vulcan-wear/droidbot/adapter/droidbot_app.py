...
class DroidBotAppConn(Adapter):
    ...
    def listen_messages(self):
        # print("\eba\ droidbot_app.py listen_messages -> port {}".format(self.port))
    ...
    def handle_message(self, message):
        ...
        # NOTE: /dcsl/ The accessible events are retrieved by an Android application installed in the device
        #       (droidApp.apk). These events are then sent via socket communication (7336) to droidbot py and
        #       received by this function (handle_message)
        ...
        if acc_event_idx >= 0:
            ...
            # print("\eba\ droidbot_app.py handle_message(): body {}".format(body))
            return
    ...
    def get_views(self):
        ...
        while not self.last_acc_event:
            # print("\eba\ droidbot_app.py get_views(): self.last_acc_event{}".format(self.last_acc_event))
        ...
        if 'view_list' in self.last_acc_event:
            # print("\eba\ droidbot_app.py get_views(): view_list {}".format(self.last_acc_event['view_list']))
