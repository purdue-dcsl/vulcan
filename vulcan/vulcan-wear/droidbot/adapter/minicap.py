...
class Minicap(Adapter):
    ...
    There is an error in the device with minicap.so library that comes with droidbot. Instead we replaced the library
    with the one that is available in the Open STF github repo
    https://github.com/openstf/minicap/blob/master/jni/minicap-shared/aosp/libs/android-26/armeabi-v7a/minicap.so
    """
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG, format=GV.LOG_FORMAT)
    minicap = Minicap()
    try:
        minicap.set_up()
        minicap.connect()
