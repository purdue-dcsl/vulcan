...
class Device(object):
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None,
                 ...
                 enable_accessibility_hard=False, humanoid=None, ignore_ad=False, is_wearable=False,
                 package_name=None, adapters=None):
        """
        initialize a device connection
        :param device_serial: serial number of target device
        :param is_emulator: boolean, type of device, True for emulator, False for real device
        :param is_wearable: boolean, type of device, True if is a wearable, False for mobile
        :return:
        """
        ...
        # instantiate the correct logcat class depending of the device
        if is_wearable:
            self.logcat = Logcat(
                device=self,
                out_file="wear-logcat.txt",
                package_name=package_name,
                is_wearable=True)
        else:
            self.logcat = Logcat(device=self)

        # FIXME. eba 6/21/19 For the wearable device probably we don't need all the adapters
        if adapters is None:
            self.adapters = {
                self.adb: True,
                self.telnet: False,
            ...
            self.adapters = {
                self.adb: adapters['adb'],
                self.telnet: adapters['telnet'],
                self.droidbot_app: adapters['droidbot_app'],
                self.minicap: adapters['minicap'],
                self.logcat: adapters['logcat'],
                self.user_input_monitor: adapters['user_input_monitor'],
                self.process_monitor: adapters['process_monitor'],
                self.droidbot_ime: adapters['droidbot_ime']
            }

        # minicap provides a socket interface for streaming screen capture data out of Android devices.
        # minicap currently not working on emulators
        # see also: https://github.com/openstf/minicap
        if self.is_emulator:
            self.logger.info("disable minicap on emulator")
            self.adapters[self.minicap] = False

        # relevant wearable information
        self.is_wearable = is_wearable
        self.sensors = []
        self.sensors = self.__get_sensors()
    ...
    def check_connectivity(self):
        ...
        for adapter in self.adapters:
            ...
            if not adapter_enabled:
                print("[CONNECTION] %s: %s is not enabled." % (self.get_model_number(), adapter_name))
            else:
                if adapter.check_connectivity():
                    print("[CONNECTION] %s: %s is enabled and connected." % (self.get_model_number(), adapter_name))
                else:
                    print("[CONNECTION] %s: %s is enabled but not connected." % (self.get_model_number(), adapter_name))
    ...
    def wait_for_device(self):
        ...
        :return:
        """
        self.logger.info("Waiting for device %s" % self.serial)
        try:
            ...
            self.logger.warning("Error waiting for device %s" % self.serial)
            import traceback
            traceback.print_exc()
        finally:
            self.logger.info("Device %s detected" % self.serial)
    ...
    def set_up(self):
        ...
        for adapter in self.adapters:
            # print("\eba\ device.py: setting adapter {}".format(adapter))
            adapter_enabled = self.adapters[adapter]
            if not adapter_enabled:
                continue
            adapter.set_up()
    ...
    def get_current_state(self):
        ...
        current_state = None
        try:
            # NOTE: /dcsl/ droidbot retrieves here the information about the device to build the current state
    ...
    def get_views(self):
        if self.cv_mode and self.adapters[self.minicap]:
            # print("\eba\ device.py get_views() using minicap")
        ...
        if self.droidbot_app and self.adapters[self.droidbot_app]:
            # print("\eba\ device.py get_views() using droidbot_app")
    ...
    def handle_rotation(self):
        ...
        if self.minicap.check_connectivity():
            print("[CONNECTION] %s: %s is reconnected." % (self.get_model_number(), self.minicap.__class__.__name__))
        self.pause_sending_event = False
    ...
    def __get_sensors(self):
        """
        Returns the list of sensors installed in the device and store them as an attribute of this class. The sensors
        are retrieved using the dumpsys command available using adb.
        :return: the list of sensors
        """
        sensors = []
        lines = self.adb.shell('dumpsys sensorservice').splitlines()
        sensors_re = re.compile('0x([A-Fa-f0-9]+)\) (.*) \| (.*) \| ver: (\d+) \| type: (.*) \| perm: (.*)')
    ...
    def get_sensor_list(self):
        """
        Returns the list of installed sensors in the device. Either invoke __get_sensors() or returns the sensors list
        stored as an attrib. in this class.
        :return: the list of sensors.
        """
        if not self.sensors:
            return self.__get_sensors()
        else:
            return self.sensors

    def __get_active_sensors(self):
        """
        Returns the list of active sensors
        :return: list of active sensors and their current connections
        """
        sensors = []
        connections = []
        lines = self.adb.shell('dumpsys sensorservice').splitlines()
        sensor_re = re.compile('(.*) \(handle=0x([A-Fa-f0-9]+), connections=(\d+)\)')

        conn_proc_re = re.compile('(.*) \| WakeLockRefCount (\d+) \| uid (\d+)')
        conn_sensor_re = re.compile('(.*) 0x([A-Fa-f0-9]+) \| status: ([A-Za-z]+) \| pending flush events (\d+)')

        active_sensor_section = False
        ...
        for line in lines:
            line = line.strip()
            if line.startswith("Active sensors:"):
                active_sensor_section = True
            elif line.startswith("Connection Number: "):
                active_sensor_section = False
                connections_section = True
            elif line.startswith("Previous Registrations:"):
                connections_section = False

            # collect info of active sensors
            if active_sensor_section:
                m_sns = sensor_re.search(line)
                if m_sns:
                    sensor = {
                        "sensor": m_sns.group(1),
                        "handle": "0x"+m_sns.group(2),
                        "connections": m_sns.group(3)
                    }
                    sensors.append(sensor)
            # collect info of connection
            elif connections_section:
                m = conn_proc_re.search(line)
                if m:
                    if conn_sensors:
                        conn_process['sensors'] = conn_sensors
                        connections.append(conn_process)
                    ...
                    m = conn_sensor_re.search(line)
                    if m:
                        conn_sensors.append("0x"+m.group(2))
    ...
    def get_sensor_status (self):
        ...
        :return: the status of the sensor in the device
        """

        sns_all_status = []
        sns_all = self.sensors
        sns_active = self.__get_active_sensors()['sensors']

        for sensor in sns_all:

            # print("\eba\ device.py - get_sensor_status: {}".format(sensor))

            # NOTE: /dcsl/ An '0x' is added to compare the handle in the lambda function. However, this could be device
            #       dependent (e.g., Huawei Watch 2)

            sns = {
                "sensor": sensor["name"],
                "handle": sensor["handle"],
                "status": 1 if (next(filter(lambda x: x['handle'] == '0x' + sensor['handle'], sns_active), 0) != 0) else 0
            }
            sns_all_status.append(sns)

        # FIXME /eba/ 7/6/19. Assuming this array (self.sensor) is sorted by handle
        # Build a signature using the handle of each sensor and its status
        status = ''.join("%s: %d " % (sns["handle"], sns["status"]) for sns in sns_all_status)

        return status


    # FIXME: /eba/ 7/6/19. This function has to be replaced by get_sensor_status.
    ...
    #     status = {}
    #
    #     # an '0x' is added to compare the handle in the lambda function; however, this could be device dependent
    #     # (e.g., Huawei Watch 2)
    #     for sensor in sns_all:
    #         sns = {
    #             "sensor": sensor["name"],
    #             "handle": sensor["handle"],
    #             "status": "active" if (next(filter(lambda x: x['handle'] == '0x' + sensor['handle'], sns_active), 0) != 0) else 'deactive'
    #         }
    ...
    # FIXME: /eba/ 7/6/19. This function has to be replaced. get_sensor_status() gets the status of sensor, now we
    #        need another function to get the status of all the states variable
    # def get_status(self):
    #     # check status of sensors
    #     sns_active = self.get_active_sensors()
    #     print("sns_active:")
    #     print (sns_active)
    #     sns_all = self.get_sensors()
    #     print("sns_all:")
    #     print(sns_all)
    #     sns_states = self.get_state(sns_active['sensors'], sns_all)
    #
    #     # convert dict to an array
    #     status = []
    #     for sns in sns_states["sensors"]:
    #         print (sns)
    #         status.append(1 if sns["status"] == 'active' else 0)
    #
    #     return status