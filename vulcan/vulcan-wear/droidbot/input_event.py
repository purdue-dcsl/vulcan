...
class InputEvent(object):
    ...
    # NOTE: /dcsl/ __dict__ is an inherit attrib. of object class, which contains all the attributes that
    #      describe the object (i.e., InputEvent)
...
class EventLog(object):
    ...
    def __init__(self, device, app, event, profiling_method=None, tag=None):
        ...
        # keep reference of the tag in the event to be able to replay it
        event.tag = tag
    ...
    def save2dir(self, output_dir=None):
        ...
        # NOTE: /dcsl/ The event is persisted in this function.
        #       Events are stored in json format in the "events" folder.
...
class UIEvent(InputEvent):
    ...
    def get_xy(x, y, view):
        ...
        # NOTE: /dcsl/ We want the center of the widget to precisely touch/click the widget and trigger the attached
        #       callback.
        if x and y:
            return x, y
        if view:
...
class TouchEvent(UIEvent):
    ...
    def send(self, device):
        ...
        # x=720
        # y=720
        print("\eba\ __send TouchEvent {}".format(self))
        print("\eba\ __send TouchEvent ({},{})".format(x, y))
