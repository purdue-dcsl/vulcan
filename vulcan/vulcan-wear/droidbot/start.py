...
def parse_args():
    ...
    parser.add_argument("-wear", action="store", dest="wear_device", required=False, choices=[0, 1, 2], type=int, const=0, nargs='?',
                        help="Declare which is the wearable device (either 0, 1 or 2)")

    # FIXME. eba 6/19/19 Remove this argument
    # for wearable devices
    # parser.add_argument("-wear", action="store", dest="wear_device_serial", required=False,
    #                     help="The serial number of the target wearable device (use `adb devices` to find)")
    # the package can be get from apk, but to avoid overhead and for future optimizations (avoid to install/reinstall
    # the apk every run) is asked as a param
    parser.add_argument("-apk", action="store", dest="wear_target_apk", required=False,
                        help="The package name of the target application")
    parser.add_argument("-no_install", action="store_true", dest="no_install",
                        help="Declare that there is no need to install the apk in the target device.")
