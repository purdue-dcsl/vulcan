...
//        }
    ...
    public String expt1 (ComponentName component, IPCType type) {
        ...
        // This is for all uri types EXCEPT "content" if needed repeat k times
        for(int i=1; i < URI_TYPES.length; i++) {
            ...
            for(Action action : specs.getActions()) {
                ...
                Log.d(TAG, String.format("expt1 | n {%d} type {%s} intent {%s} uri {%s} action {%s}",
...
//        {
//            Uri u = ug.getUri(URI_TYPES[0], rnd.nextInt(mKnownProviders.size())); //index used to be i //20111203
